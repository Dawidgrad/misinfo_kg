...
class Classification:
    ...
    def ukraine_misinfo(self):
        ...
        new_filename = f'{self.working_dir}\source\input_files\\claims_{file_idx}.txt'
        ...
        args = ['java', '-mx8g', '-cp', self.stanford_path, 'edu.stanford.nlp.naturalli.OpenIE', '-filelist', 
                ...
                f'{self.working_dir}\source\output_files\openie_output_claims.txt', '-tokenize.options', 'untokenizable=noneDelete']
        args = ' '.join(args)
        ...
        # Make sure a new file is created to avoid appending to the output from previous run
        new_filename = f'{self.working_dir}\source\input_files\\titles_{file_idx}.txt'
        silent_remove(new_filename)
        file = open(new_filename, 'a', encoding='utf-8')
        ...
        args = ['java', '-mx8g', '-cp', self.stanford_path, 'edu.stanford.nlp.naturalli.OpenIE', '-filelist', 
                f'{self.working_dir}\source\input_files\\filelist.txt', '-output',  
                f'{self.working_dir}\source\output_files\openie_output_titles.txt', '-tokenize.options', 'untokenizable=noneDelete']
        args = ' '.join(args)
